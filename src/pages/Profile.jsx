...
export default function Profile() {
    ...
    const onDelete = async(theId)=>{
        if (window.confirm('Are you sure you want to delete?')) {
            await deleteDoc(doc(db,'listings',theId));
            const updatedListings = listings.filter((listing)=> listing.id !== theId);
            setListings(updatedListings);
            toast.success('Successfully deleted the listing')
        }
    }
    const onEdit = (theId)=>{
        navigate(`/edit-listing/${theId}`)
    }
